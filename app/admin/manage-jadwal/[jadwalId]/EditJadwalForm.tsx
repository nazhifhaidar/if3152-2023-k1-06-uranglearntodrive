  ...
  useEffect(() => {
    const fetchOptions = async () => {
        try {
            ...
            setJadwal(data?.data); 
            setIdKelas(jadwal.id_kelas);
            if(jadwal.tanggal !== undefined){
                const tanggal = jadwal.tanggal.toString().split("T")[0];
                setTanggal(tanggal);
                setSelectedDate(new Date(`${tanggal}T07:00:00`));
            }
            if(jadwal.start_sesi !==undefined){
                setSelectedStartSesi(jadwal.start_sesi.toString().split("T")[1].split(".")[0].split(":")[0]);
            }
            if(jadwal.end_sesi !== undefined){
                setEndSesi(jadwal.end_sesi.toString().split("T")[1].split(".")[0].split(":")[0]);
            }
        ...
        fetchOptions();
    }, [params.id, jadwal.id_kelas, jadwal.tanggal, jadwal.end_sesi, jadwal.start_sesi]);
  ...
  return (
      <div style={{width:'max-content'}}>
        ...
        <form onSubmit={handleSubmit} >
            <DropdownInputKendaraan Dropdownlabel="Nama Kelas" Dropdownname="id_kelas" DropdownValue = {id_kelas} TextLabel="Id Kelas" TextName="idkelas" TextValue={id_kelas} Loading = {loading} Options={optionsKelas} onSelect={handleSelectKelas} ></DropdownInputKendaraan>
